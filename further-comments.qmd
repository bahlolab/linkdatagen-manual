...
Stipulation of this option (`-removeWFHBS`) means that within-family homozygosity-by-state markers, i.e. markers that have missing or identical homozygous genotypes across all samples within a family, are removed prior to marker selection. 
Such markers are deemed "uninformative" for some analyses. To clarify, different families may be homozygous for different alleles for a marker to become uninformative. When dealing with multiple families, two values are available; "u" (for union) and "i" (for intersection). With value "u", a marker that is uninformative in ANY family is removed. With value "i", a marker that is uninformative in ALL families is removed. Value "u" is the default and will generally produce more informative markers across all families. Value "i" may be useful in a number of settings, in particular when quality of genotypes is poor such that using value "u" would wipe out the majority of genotypes in all families.
